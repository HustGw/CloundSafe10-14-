...
#include "get_packet.h"

typedef struct bit_stream_dec Bitstream;
#define NALU_PREFIX_LEN 3

extern AVBitStreamFilterContext* h264bsfc;
...
typedef struct  
{
	char srcfilepath[FILE_NAME_SIZE];
	char keyfilepath[FILE_NAME_SIZE];
	int FrameInterval;
	int average_extract_Nalu;
	double extract_Nalu_percent;
	int average_extract_Islice;
	double extract_Islice_percent;
...
typedef struct inp_par
{
	...
	double nalu_percent;
	int average_extract_Islice;
	double Isclice_percent;
    int FileFormat;                         //!< File format of the Input file, PAR_OF_ANNEXB or PAR_OF_RTP
    int silent;
	int FrameInvl;		//frame interval

	int interval;
	int is_decode_one_pbframe;
