...
#pragma mark - 拍照与拍摄 add by Alan on Sep. 22
@property (nonatomic, assign) BOOL isTakePhone;
@end
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<TZImagePickerControllerDelegate>)delegate AssetType:(TZAssetModelMediaType)assetType isTakePhoto:(BOOL) isTakePhoto
{
    self.assetType = assetType;
    self.isTakePhone = isTakePhoto;
    return [self initWithMaxImagesCount:maxImagesCount delegate:delegate];
}
...
- (void)pushToPhotoPickerVc {
    ...
    if (_pushToPhotoPickerVc) {
        TZPhotoPickerController *photoPickerVc = [[TZPhotoPickerController alloc] initWithIsTakPhoto:self.isTakePhone];
        ...
        [[TZImageManager manager] getCameraRollAlbum:self.allowPickingVideo allowPickingImage:self.allowPickingImage completion:^(TZAlbumModel *model) {
            ...
            photoPickerVc.assetType = self.assetType;
...
//        [backButton setImage:[UIImage imageNamed:@"backIcon"] forState:UIControlStateNormal];
        ...
        backButton.tintColor = [UIColor colorWithRed:17.0/255.0 green:205.0/255.0 blue:110.0/255.0 alpha:1.0];
