...
@property (nonatomic, assign) BOOL isTakePhoto;//add by Alan on Sep. 22
...
#pragma mark - 初始化方法 add by Alan
- (id)initWithIsTakPhoto:(BOOL) isTakePhoto
{
    if (self = [super init]) {
        self.isTakePhoto = isTakePhoto;
    }
    return self;
}
    ...
    if (!tzImagePickerVc.sortAscendingByModificationDate && _isFirstAppear && iOS8Later) {
        ...
        if (_showTakePhotoBtn || !iOS8Later || _isFirstAppear) {
            [[TZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<TZAssetModel *> *models) {
                ...
                _models = [NSMutableArray arrayWithArray:[self getAssetArr:models]];
...
///根据资源类型返回资源数组
- (NSArray *)getAssetArr:(NSArray *)assetArr
{
    if (self.assetType == TZAssetModelMediaTypePhoto)
    {
        NSMutableArray *photoArr = [[NSMutableArray alloc]init];
        for (TZAssetModel *assetModel in assetArr)
        {
    ...
    if (self.assetType == TZAssetModelMediaTypeVideo)
    {
        NSMutableArray *vedioArr = [[NSMutableArray alloc]init];
        for (TZAssetModel *assetModel in assetArr)
        {
            if (assetModel.type == TZAssetModelMediaTypeVideo)
            {
                [vedioArr addObject:assetModel];
            }
        }
        return vedioArr;
    }
...
- (void)okButtonClick {
#pragma mark - start add by Alan 删除原文件
    [self okButton];
//    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示" message:@"是否删除原文件？" preferredStyle:UIAlertControllerStyleAlert];
//    UIAlertAction *ok = [UIAlertAction actionWithTitle:@"是" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
//        [self okButton];
//        [[NSNotificationCenter defaultCenter] postNotificationName:@"deleteOriginal" object:@"YES"];
//    }];
...
//        [self okButton];
//        [[NSNotificationCenter defaultCenter] postNotificationName:@"deleteOriginal" object:@"NO"];
//    }];
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    }else if ([type isEqualToString:@"public.movie"])
    {
        ...
        TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.navigationController;
        [imagePickerVc showProgressHUD];
        [[TZImageManager manager] saveVideoWithURL:videoURL completion:^{
            TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
            [[TZImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(TZAlbumModel *model) {
                _model = model;
                [[TZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<TZAssetModel *> *models) {
